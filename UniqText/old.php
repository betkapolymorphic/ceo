...
function getFrequencyWords($text)
{
    $sklonen = array();
    ...
    foreach ($words as $word) {
        ...
        if ($flag) {
            $sklonen[$word1]['sklonen'] = getNewFormText($word1);
        }
        return $sklonen;
    }
    function correctText($text)
    {
        ...
        $frequencyWords = getFrequencyWords($text);
                    ...
                    if (!in_array($word, $badWord)) {
                        ...
                        if ($pos !== false) {
                            $text = substr_replace($text, $synonyms[$synonyms_counter]
                                , $pos, strlen($word));
                            array_push($badWord, $word);
                            echo "$word => $synonyms[$synonyms_counter]<br>";
                            $synonyms_counter++;
                            break;
                        }
                        ////Конец замены
    ...
    echo json_encode(getFrequencyWords("женщина танец женщин"));
